...
class Solution:
# Time: O(2**N) where N is the number of parentheses. For each parenthes, we either keep it or discard it,
...
def removeInvalidParentheses(self, s: str) -> List[str]:
    ...
    while q:
        ...
        for cur in q:
            if self.isValid(cur):
                if not res or len(cur) > len(res[0]):
                    res = [cur]
                elif len(cur) == len(res[0]):
                    res += [cur]
        ...
        # if all the expressions is not valid, for each of which, remove more
        s = set()
